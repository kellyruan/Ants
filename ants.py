...
class Bee(Insect):
    ...
    def action(self, colony, exit = True):
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.contained_ant:
            self.action = self.contained_ant.action(colony)
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            ants_place = self.place.exit
            while ants_place:
                if ants_place.ant:
                    if ants_place.ant.is_container:
                        if ants_place.ant.contained_ant not in self.doubled_ants:
                            if isinstance(ants_place.ant.contained_ant, ThrowerAnt):
                                ants_place.ant.contained_ant.damage *= 2
                            self.doubled_ants.append(ants_place.ant.contained_ant)
                    if ants_place.ant not in self.doubled_ants:
                        if ants_place.ant.damage:
                            ants_place.ant.damage *= 2
                        self.doubled_ants.append(ants_place.ant)
                ants_place = ants_place.exit
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    modified_action = effect(bee.action, bee)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target:
            if target not in self.scared_bees:
